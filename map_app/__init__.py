...
def compute_mean_duration (df, cut_off_date):
    ...
    #set cut off date to the last date when not specified
    if cut_off_date == 'max':
        cut_off_date = df.date_powerfailure.max()
    
    print('Check Data Type of DF going into func ',type (df))
...
def predict_power_states():
    if request.method == "POST":
        ...
        df_avg = compute_mean_duration (df_pwr_out, pred_date)

        #Make the prediction using target = xy, 
        pred_5_wmean = k_nearest_neighbors(df_avg, xy, 5,'wmean')
...
@app.route('/csv_downloads/')
def csv_downloads():
    return render_template("downloads.html")

@app.route('/return_csv_daily_powerout_cnts/')
def return_csv_daily_powerout_cnts():
    return send_file(APP_ROOT + '/input/powerout.csv', mimetype='text/csv', 
			attachment_filename="daily_powerout_counts.csv",
			 as_attachment=True)

@app.route("/boxes")
def add_boxes_location():
    """Get location of the boxes and them to the map"""
    pwr_out_file = APP_ROOT + '/input/powerout.csv' 
    df_pwr_out = preprocess_powerout_data(pwr_out_file)
    df_avg = compute_mean_duration (df_pwr_out, 'max')
    print (df_avg.size)
    pts = data2geojson(df_avg)
    return jsonify(pts)
...
if __name__ == "__main__":
    ...
    #df_pwr_out = preprocess_powerout_data (pwrout_file)
